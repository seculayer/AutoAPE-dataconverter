...
class Dilation(ConvertAbstract):
    ...
    def apply(self, data: Any) -> List:
        ...
        dilation = _cv2.dilate(img, kernel, iterations=self.iterations)
        _1to3channel = np.repeat(dilation[:, :, np.newaxis], 3, -1)
...
if __name__ == "__main__":
    ...
    with open(f"{image_path}/noise_img.png", "rb") as img_f:
        ...
        rst = Dilation(stat_dict=None, arg_list=[2]).apply(img_data)

        print(rst[0])
    __cv2.imwrite(f"{image_path}/dilation_rst.jpg", rst[0])
